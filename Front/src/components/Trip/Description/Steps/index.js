...
const Steps = ({
    ...
    const handleShow = (step) => {
      setValues({
        ...values,
        localisation: [step.latitude, step.longitude],
      });
      const showTest = show.slice();
    ...
    const handleCloseDelete = (modalId) => {
      const showTest = showDelete.slice();
      showTest[modalId] = false;
    ...
    const handlePictures = (e) => setValues({
      ...values,
    ...
    const DraggableMarker = () => {
      ...
      return ( <
        ...
        <
        Popup > Position de votre étape.Déplacez le marqueur pour affiner la position. <
        /Popup> <
        /Marker>
      );
    };
    ...
    const useGeocodingApi = (event) => {
      ...
      const APIkey = '633a0b0f87f60b5b289a3c5aca315d0c';
    ...
    const locateSuggestion = () => {
      const adressSelected = suggestions.find((suggestion) => suggestion.label == values.localisationInput) || suggestions[0];
    ...
    const getCountryFromAPI = () => {
      ...
      axios.get(`http://api.positionstack.com/v1/reverse?access_key=${APIkey}&query=${reverseQuery}`)
        .then((response) => {
          ...
          setValues({
            ...values,
            country_code: currentCountry_code,
          });
        })
        .catch((error) => {
          console.error(error);
    ...
    return ( <
      div > {
        ...
        ( <
          ...
          Col className = "nav-container" > {
            steps.map((step, index) => {
              ...
              return ( <
                ...
                <
                Nav.Link href = {
                  sluggedTitleAsAnchor
                }
                className = "step-numbers" >
                {
                  index + 1
                } <
                /Nav.Link> <
          ...
          <
          /Col> <
          /Row> <Row className="steps_container"> <
          Col > {
            ...
            steps.map((step) => {
                  ...
                  const sluggedTitle = slugify(step.step_title, {
                    lower: true,
                  });
                  ...
                  return ( <
                      ...
                      <
                      /CardColumns> <Card.Body> <
                      Card.Title className = "card-step-title" > {
                        step.step_title
                      } <
                      /Card.Title> <Card.Subtitle className="card-step-date"> {
                      dayjs(`${step.step_date}`).format('DD/MM/YYYY')
                    } <
                ...
                trip.author[0].id === connectedUserId && ( <
                  ...
                  <
                  Button className = "edit-step-button"
                  onClick = {
                    () => {
                      handleShow(step);
                    }
                  ...
                  <
                  i className = "fas fa-pencil-alt" / > Editer l 'étape <
                  /Button> <Button
                  variant = "danger"
                  className = "delete-step-button"
                  onClick = {
                    () => {
                      handleShowDelete(step.id_step);
                    }
                  } >
                  <
                  i className = "fas fa-trash-alt" / > Supprimer l 'étape <
                  /Button> <
                  /div>
                )
              } <
            ...
            className = "form-title" >
              <
              Modal.Title > Suppression de votre étape <
              /Modal.Title> <
              /Modal.Header> <Modal.Body> <
              p > Êtes - vous sûr de vouloir supprimer votre étape ? Toute suppression est irréversible. <
              /p> <
              /Modal.Body> <Modal.Footer className="form-title"> <
              Button
            variant = "danger"
            className = "m-2"
            onClick = {
            ...
            className = "form-title" >
              <
              Modal.Title > Modifier les infos de votre étape <
              /Modal.Title> <
              /Modal.Header>
            ...
            controlId = "title" >
              <
              Form.Label > Titre de votre étape <
            ...
            controlId = "summary" >
              <
              Form.Label > Description de votre étape <
              /Form.Label> <Form.Control
            name = "summary"
            as = "textarea"
            rows = {
        ...
        className = "form-button form-button-gps" >
        ...
        /div> {
        values.showInput && ( <
          InputGroup > < Form.Control name = "localisationInput"
          type = "text"
          defaultValue = ""
          onChange = {
            (e) => {
        ...
        /datalist> <InputGroup.Append> <Button
...
controlId = "date" >
  <
  Form.Label > Date de cette étape <
  /Form.Label> <Form.Control
name = "date"
type = "date"
defaultValue = {
  dayjs(`${step.step_date}`).format('YYYY-MM-DD')
}
onChange = {
  (e) => handleChange(e)
}
